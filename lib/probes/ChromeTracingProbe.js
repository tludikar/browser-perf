// Deprecated, not used by may metric
var util = require('util'),
	events = require('events'),
	Q = require('q'),
	wd = require('wd'),
	helpers = require('../helpers'),
	debug = require('debug')('bp:probes:ChromeTracingProbe');
...
ChromeTracingProbe.prototype.isEnabled = function(browser) {
	return this.browsers.filter(function(allowedBrowser) {
		return browser.platform !== 'ANDROID' && browser.browserName.match(new RegExp(allowedBrowser, 'i'));
...
ChromeTracingProbe.prototype.startRecordingTrace_ = function(browser) {
	...
	return browser.windowHandle().then(function(handle) {
		...
		return browser.execute(helpers.fnCall(function() {
			window.open('about:blank', 'bpTracing', 'menubar=no');
		}));
	}).then(function() {
		return browser.window('bpTracing');
	}).then(function() {
		...
		return browser.execute(helpers.fnCall(function() {
			document.getElementsByClassName('record-dialog-overlay')[0]['currentlyChosenPreset'] = []
...
ChromeTracingProbe.prototype.stopRecordingTrace_ = function(browser) {
	...
	return browser.windowHandles().then(function() {
		...
		debug(' Finished ChromeTracingProbe data ' + me.originalWindow);
		return browser.window(me.originalWindow);
	});
};
