...
ChromeTracingMetrics.prototype.onData = function(data) {
	if (data.type === 'perfLog') {
		...
		data.value.forEach(function(msg) {
			if (msg.method === 'Tracing.dataCollected' && /\b(benchmark|__metadata)\b/.test(msg.params.cat)) {
				this.tracingStats.push(msg.params);
			}
			if (msg.method === 'Tracing.bufferUsage') {
				debug('Error recording tracing data - buffer usage exceeded', msg);
			}
		}, this);
